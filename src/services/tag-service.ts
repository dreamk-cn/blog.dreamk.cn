...
export async function listTags(keyword = "") {
  ...
  if (keyword) {
    query.where = taxonomyKeywordWhere(keyword) as Prisma.TagFindManyArgs['where'];
  }
...
export async function createTag(input: { name: string; slug?: string }) {
  ...
  const exists = await taxonomyConflictExists(prisma.tag, input.name, normalizedSlug);
...
export async function updateTag(input: { id: string; name: string; slug?: string }) {
  ...
  const normalizedSlug = normalizeSlug(input.slug || input.name, 50);
  const exists = await taxonomyConflictExists(prisma.tag, input.name, normalizedSlug, input.id);
